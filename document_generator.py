...
def bold_runs(text, para, size, color, font="Calibri"):
    for p in re.split(r"(\*\*.*?\*\*)", text):
        run = para.add_run(p[2:-2] if p.startswith("**") and p.endswith("**") else p)
        run.bold = p.startswith("**")
...
def parse_cover_letter(text):
    ...
    def not_salutation(l): return not l.lower().startswith("dear")

    if idx < len(non_empty) and not_salutation(non_empty[idx]):
        data["sender_name"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and not_salutation(non_empty[idx]):
        data["sender_address"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and not_salutation(non_empty[idx]) and not "@" in non_empty[idx] and not non_empty[idx][0].isdigit():
        data["sender_city"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and non_empty[idx][0].isdigit():
        data["sender_phone"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and "@" in non_empty[idx]:
        data["sender_email"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and any(m in non_empty[idx] for m in ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","20"]):
        data["date"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and not_salutation(non_empty[idx]):
        data["recipient_name"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and not_salutation(non_empty[idx]):
        data["recipient_title"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and not_salutation(non_empty[idx]):
        data["company_name"] = non_empty[idx]; idx += 1
    if idx < len(non_empty) and not_salutation(non_empty[idx]):
        data["company_address"] = non_empty[idx]; idx += 1
    ...
    if sal_idx is not None:
        ...
        for line in lines[sal_idx+1:]:
            ...
            if low in ["sincerely,","sincerely","regards,","regards","best regards,","warm regards,"]:
                if current: para.append(" ".join(current)); current = []
                data["closing"] = line
                break
            elif line.strip() == "":
                if current: para.append(" ".join(current)); current = []
        ...
        if current: para.append(" ".join(current))
        data["paragraphs"] = [p for p in para if p.strip()]
...
def generate_cover_letter_docx(text, output_path, company):
    ...
    def line(txt, bold=False, size=12, space_after=0, italic=False, color=None, align=None):
        p = doc.add_paragraph()
    ...
    if d["sender_name"]:    line(d["sender_name"], bold=True)
    if d["sender_address"]: line(d["sender_address"])
    if d["sender_city"]:    line(d["sender_city"])
    if d["sender_phone"]:   line(d["sender_phone"])
    if d["sender_email"]:   line(d["sender_email"])
    blank(12)
    line(d["date"])
    blank(12)
    if d["recipient_name"]:   line(d["recipient_name"], bold=True)
    if d["recipient_title"]:  line(d["recipient_title"])
    if d["company_name"]:     line(d["company_name"])
    if d["company_address"]:  line(d["company_address"])
    blank(12)
    line(d["salutation"] or "Dear Hiring Manager,")
    blank(10)

    for para_text in d["paragraphs"]:
        p = doc.add_paragraph()
    ...
    blank(12)
    line(d["closing"] or "Sincerely,")
    blank(36)
    if d["sender_name"]:
        line(d["sender_name"])
        p = doc.add_paragraph()
        r = p.add_run(d["sender_name"])
        r.italic = True
        r.font.name = "Times New Roman"
        r.font.size = Pt(12)
...
def generate_cover_letter_pdf(text, output_path, company):
    ...
    if d["sender_name"]:    s += [Paragraph(safe(d["sender_name"]), B)]
    if d["sender_address"]: s += [Paragraph(safe(d["sender_address"]), N)]
    if d["sender_city"]:    s += [Paragraph(safe(d["sender_city"]), N)]
    if d["sender_phone"]:   s += [Paragraph(safe(d["sender_phone"]), N)]
    if d["sender_email"]:   s += [Paragraph(safe(d["sender_email"]), N)]
    s.append(SP(14))
    s += [Paragraph(safe(d["date"]), N), SP(14)]
    if d["recipient_name"]:   s += [Paragraph(safe(d["recipient_name"]), B)]
    if d["recipient_title"]:  s += [Paragraph(safe(d["recipient_title"]), N)]
    if d["company_name"]:     s += [Paragraph(safe(d["company_name"]), N)]
    if d["company_address"]:  s += [Paragraph(safe(d["company_address"]), N)]
    s.append(SP(14))
    s += [Paragraph(safe(d["salutation"] or "Dear Hiring Manager,"), N), SP(10)]
    for pt in d["paragraphs"]:
        if pt.strip(): s.append(Paragraph(safe(pt.strip()), BD))
    s.append(SP(14))
    s += [Paragraph(safe(d["closing"] or "Sincerely,"), N), SP(40)]
    if d["sender_name"]:
        s += [Paragraph(safe(d["sender_name"]), N), Paragraph(f"<i>{safe(d['sender_name'])}</i>", IT)]
